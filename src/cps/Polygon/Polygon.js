...
export default class Polygon extends Graph {
  ...
  toEdit () {
    ...
    this.ent.polygon.hierarchy = new Cesium.CallbackProperty((time, result) => {
      return new Cesium.PolygonHierarchy(this.calcuteShape(this.graph.ctl._children, time), [])
    }, false)
  }
  ...
  finish () {
    if (this.ent) {
      super.finish()
      this.ent.polygon.hierarchy = new Cesium.PolygonHierarchy(this.calcuteShape(this.graph.ctl._children, mu.julianDate()), []) 
    }
  }
}
